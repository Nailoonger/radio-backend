...
- **周状态带（2026-09-27，commit `1a274de`）**：文案规则＝**一个阶段只有两种形态** —— 进行中带「中」
  （点播中/审核中/排期中），**已完成与未到都不带**（点播/审核/排期）；**状态只由胶囊颜色表达**
  （深 `--ink`＝当前 / 绿＝已走过 / 羊皮纸＝未到 / 浅灰＝已取消）。`WEEK_FLOW` 存 `cn`+`on` 两个名，
  `cn: isOn ? x.on : x.cn` 切换。后端 `WEEK_STATUS_CN` 是**"进行中"口径**（`APPLICATION:'点播中'`），
  去「中」由前端做；`DRAFT` 两端统一「未开放」。
  ⚠️⚠️ **前端 `WEEK_FLOW` 必须与后端 `WEEK_STATUS` 六态对齐**（`DRAFT/APPLICATION/REVIEW/SCHEDULING/LOCKED/CANCELLED`）：
  曾漏 `CANCELLED` → `findIndex` 返回 −1 → **整条链全落「未到」**（被取消的周像"还没开始"），静默错不报错。
- **术语定名「收歌」→「点播」（同批）**：只改**展示名**，后端常量仍是 `APPLICATION`，接口/数据零改动。
  ⚠️ **改名会打断验证脚本里的断言期望**（脚本会掉项）→ 改名类改动**必须连脚本一起换 + 重跑**，不能只看构建。
  仍未改的「收歌」＝ docs / 历史 preview / `sql` 列 COMMENT / 两处代码注释（有意保留，与下次 DB 变更一批做）。
- **投稿列表（SubmitList）列结构**（2026-09-27 实测，1920 视口）：7 列 —— selection 46 / 内容
  `min-width:330` 弹性列**实得 800** / 投稿人 150 / 提交时间 118 / 审核人·时间 142 / 状态 148 /
  操作 236（`fixed="right"`）。**没有空列** —— 页面中间那块"有行分隔线、没内容"的区域是内容列的右半截。
  ⚠️ 改列表页前**先用 `getBoundingClientRect()` 逐列量**，别拿截图猜列宽。
  ⚠️ **操作列 236 是算出来的**：最长组合「指派时段 78 + 改驳回 66 + 删除 28 + gap 12」= 184px，
  加 td padding 20 → 204，取 236 留余量；**旧值 200 会把「查看日志」裁成「查看」**。
  判断"用户报的裁切是不是当前 bug"→ 先看**按钮组合对不对得上代码**：超管在「已排期」下不渲染
  「查看日志」，服务器截图里出现了它 ＝ 更早的版本（2026-09-27 就是这么定位的）。
- **陛下发来的批注截图**：用 skill `screenshot-annotation-parse` 解析
  （扫暗红像素 → 直方图找框边界 → 裁开放大读手写）—— 别肉眼猜他圈的是哪一列，猜错就是整轮返工。
