...
> ② **收歌截止 ≠ 审核截止**，审核截止独立配置，`schedule_lock_at` 改由它派生。
...
## 八、2026-09-25 点歌时间窗口改版（陛下裁决：周内任选 + 审核截止独立）

陛下原话：「点歌时间窗口改为自定义周内所有时间段」→ 选项式确认后定为
**① 单段 · 周内任选**、**② 审核截止拆成独立一项**。
...
| | 旧 | 新 |
| --- | --- | --- |
| 收歌开始日 | 只能 周五 / 周六 / 周日 | **周一 → 周日 任选** |
| 收歌结束日 | 只能 周六 / 周日 | **周一 → 周日 任选** |
| 最长跨度 | 72 小时 | **可铺满整周**（周一 00:00 → 周日 23:59，`HH:mm` 表示不了 24:00） |

旧白名单存在的唯一理由是「保证窗口不跨播出周周一 00:00」——
放宽后这条**改由公式结构性保证**：`offMon(day) ∈ [0,6]` + `HH:mm < 24:00`
⇒ 最晚偏移 = 周日 23:59 < 7 天，天然不跨。

### 8.2 ⚠️ 锚点必须换（这是这次最容易踩的坑）

旧实现从**播出周**往回推：`offBack(d) = (1-d+7)%7` → `offBack(周一) = 0`，
于是 `startDay=周一` 的窗口落到了**播出周本身**。一旦窗口覆盖到播出周的周一 00:00 之后，
`nextWeekRange(now)` 就会指向「再下一周」，同一窗口里的提交被算成投给不同的周。

新实现锚在**收歌周**（= 播出周 − 7 天）：
`offsetOf(day,time) = ((day===0?7:day)−1) * 1天 + HH:mm`。
对旧白名单三值 `offMon(d) === 7 − offBack(d)`，**算出的绝对时刻完全一致**（升级不改行为），
对周一~周日 7 值则全部落在收歌周内。

硬约束只剩 `0 ≤ startOff < endOff ≤ 7 天` +「不跨播出周周一 00:00」。

### 8.3 审核截止独立

- 新增 `reviewDay` / `reviewTime`（KV `song_submit_window`，`null` = 未单独配置）。
- `weekly_schedule.schedule_lock_at` / `review_end_at` 改由审核截止派生，**不再 = 窗口结束 + 360min**。
- 校验：审核截止 ≥ 收歌结束、且 ≤ 播出周周一 00:00。
- **兼容**：未配置时退回「收歌结束 + `song_lock_offset_minutes`」（默认 360 分）→
  默认窗口下仍是播出周周一 00:00；窗口限制关闭时退回播出周周一 00:00 兜底。

### 8.4 前端同步（本次一起做）

| 位置 | 改动 |
| --- | --- |
| `SongSettings.vue` | 星期下拉放开全周（`allowedDays`）、新增独立「审核截止」行（含「跟随收歌结束 + N」档）、文案改「收歌截止 ≠ 审核截止」、4 个保存按钮按角色置灰 |
| `SubmitList.vue` | 执行排期 / 模拟排期 / 锁定 / 改时段 / 指派 / 标记播放 / 撤销 **按角色显隐**（普通人看到只读提示）；矩阵「空位」文案按闸门切「不外借 ↔ 可填」；**新增模拟排期面板**接 `POST /submit/schedule/preview` |
| `submit.js` / `windowBar.js` / `submit.wxml` | 回执屏「审核截止」改取 `reviewAt`（不再用收歌截止顶替）；锁定时刻文案去掉「播出周周一」硬编码 |

### 8.5 验证

`node scripts/verify-song-protocol.js` → **158 项全过**（新增 Q 节 28 项：锚点、新旧公式等价、
