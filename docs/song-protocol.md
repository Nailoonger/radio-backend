...
审核
   ...
   └─ 通过 → review=APPROVED, schedule=UNASSIGNED
          ...
          └─ 触发 RescheduleAllocator
                 ...
                 · 目标格：首选仍空 → 放首选（记 PROMOTED）；否则放最近的空位（记 RESCHEDULED）
...
周内时段序列（周一到周五 × 每日时段）的下标差。例：周一午间 vs 周二午间，隔 3 个时段 → 距离 3。
首选时段永远距离 0，所以「首选优先」是这条规则的自然结果，不用单独写特例。

### 3.2 幂等与并发

- 所有状态变更都是**条件 UPDATE**（`WHERE status = ...`），重复跑只少做事。
