 ...
 * PUT    /api/admin/member/:id/toggle
 *
...
exports.create = async (req, res, next) => {
  try {
    const { name, role, grade, programs, avatar, motto, sort, isShow } = req.body || {};
    if (!name) throw new ApiError(Codes.PARAM_ERROR, '请填写姓名');
    if (!role) throw new ApiError(Codes.PARAM_ERROR, '请填写职务');
    if (!avatar) throw new ApiError(Codes.PARAM_ERROR, '请上传或填写头像 URL');
...
exports.update = async (req, res, next) => {
  try {
    ...
    const fields = ['name', 'role', 'grade', 'programs', 'avatar', 'motto', 'sort', 'isShow'];
