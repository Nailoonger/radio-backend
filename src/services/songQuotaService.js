...
/**
 * ⛔ @deprecated 自 2026-09-20（点歌规则 v2，docs/song-queue-v2.md）起退役。
 *
 *    为什么退役：本模块记的是「审核那一天/那一周过了几条」，而学生投的稿锁的是
 *    「下周周一到周五」—— 口径错位（管理员集中在周末审，日名额毫无意义）。
 *    更要命的是它是一本独立计数器，与真实数据两本账，于是有
 *    「驳回已通过件不还名额」这类泄漏。
 *
 *    v2 的做法：不再用计数器 —— 占位就是 submit 行本身
 *      （status ∈ {0,1,4} AND scheduled_slot = 某个格子），
 *    容量与候补队列全部由 `src/services/songQueueService.js` 负责。
 *
 *    ⚠️ 本文件保留只为「历史代码/老脚本引用不报错」，**新代码不要 import 它**。
 *    表 `song_quota` 已停写并在迁移里清空，v2.x 会 DROP。
 *
 * ---------------------------------------------------------------------------
 * 点歌名额服务 —— 日 / 周 名额的原子占用、自动驳回、快照（v1，已退役）
 *
 * ═══════════════════════════════════════════════════════════════════════
