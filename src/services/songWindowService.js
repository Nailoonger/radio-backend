 ...
 *   · **点播截止 ≠ 审核截止**：点播结束只停止收新歌；审核截止单独配（reviewDay/reviewTime），
 *     到点才自动排期 + 驳回剩余候补 + 锁定本周
 ...
 *     songWeek   = weekStart - 7 天                // 点播周的周一 00:00（窗口只允许落在这里面）
 *     offMon(d)  = ((d===0 ? 7 : d) - 1)           // 周一→0 … 周日→6（从点播周周一起算的天数）
 ...
 *
 *   硬约束（保命）：`0 <= startOff < endOff <= 7 天`，即窗口不跨点播周周一 00:00。
...
/** 审核截止未单独配置时的兜底偏移：点播结束 + N 分钟（沿用历史口径） */
 ...
 * reviewDay / reviewTime = null 表示「审核截止未单独配置」→ 退回 `点播结束 + 偏移`。
...
/** 窗口 / 审核截止的偏移上限：不得跨过播出周周一 00:00（7 天 = 点播周整周） */
...
/** 从「点播周周一 00:00」起算的偏移天数：周一→0、周二→1 … 周日→6（入参是 0=周日 的编码） */
function offMon(day) {
  const d = parseDay(day, 1);
  return (d === 0 ? 7 : d) - 1;
}

/** 星期 + 时刻 → 相对点播周周一 00:00 的毫秒偏移（窗口与审核截止共用） */
...
async function setConfig(patch, adminId) {
  ...
  if (merged.enabled) {
    // ② 窗口自身：不跨点播周周一 00:00（offMon ≤ 6 + HH:mm < 24:00 ⇒ 结构上恒成立，这里仍显式兜一道）
    ...
    if (startOff >= endOff) {
      throw new ApiError(
        Codes.PARAM_ERROR,
        `点播结束必须晚于点播开始（当前：${describeOffset(startOff)} → ${describeOffset(endOff)}）`
      );
    }
  }

  // ③ 审核截止：不得早于点播结束、不得晚于播出周周一 00:00
  if (merged.reviewDay !== null) {
    if (reviewOff < endOff) {
      throw new ApiError(
        Codes.PARAM_ERROR,
        `审核截止不得早于点播结束（当前：点播结束 ${describeOffset(endOff)} / 审核截止 ${describeOffset(reviewOff)}）`
      );
    }
 ...
 * ------------------------------------------------------------------ */
/**
 * 审核截止相对点播周周一 00:00 的偏移
 *   ① 配了 reviewDay + reviewTime → 用配置值（新形态，管理员显式指定）
 *   ② 没配 + 窗口开启 → 点播结束 + `song_lock_offset_minutes`（默认 360 分）—— 兼容旧口径，
...
/**
 * 窗口区间 —— 锚点＝**点播周的周一 00:00**（= 目标播出周周一 00:00 − 7 天）
 * 窗口整体落在点播周内 ⇒ 窗口内任意时刻 `nextWeekRange(now)` 恒等于同一个 weekStart，
 * 「投的是哪一周」永不跳变。
 */
function windowRangeAt(cfg, now = Date.now()) {
  const weekStart = bj.nextWeekRange(now).start;                 // 目标播出周的周一 00:00（绝对时刻）
  const songWeek = new Date(weekStart.getTime() - 7 * DAY_MS);   // 点播周的周一 00:00
  ...
  return {
    ...
    songWeek,                                                    // 点播周周一 00:00
  };
}

/**
 * 完整锚点 = 点播开始 / 点播结束 / 审核截止（周行落库、管理端展示统一走这里）
 */
...
/** 审核截止文案：配了就报配置值，否则报实际生效的时刻（「点播结束 + 偏移」算出来的那天） */
...
async function status(now = Date.now()) {
  ...
  const base = {
    ...
    reviewAt: toBjsIso(reviewAt),        // 独立审核截止（点播结束后才开始审这一轮）
    opensAt: null,
    closesAt: null,
    secondsToOpen: null,
    secondsToClose: null,
...
async function describe(now = Date.now()) {
  ...
  return {
    ...
    /** 审核截止是否被单独配置过（false = 跟随「点播结束 + 偏移」，升级前后行为一致） */
    ...
    note: '点播截止只停止新提交；到审核截止系统才自动排期、把剩余候补驳回并锁定本周',
  };
}
